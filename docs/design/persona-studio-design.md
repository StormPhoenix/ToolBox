...
interface PersonaMeta {
  ...
  /**
   * 人格来源类型（可选，缺省视为 'distilled'，兼容旧数据）。
   * - 'distilled'：通过材料 + 配方蒸馏产生
...
导入型 `recipe_name` 存空字符串，`sources` 为空数组。代码层不感知 SKILL.md 的内容结构，内容 schema 完全由配方决定。
...
点击 "+ 新建人格" 弹出 `CreatePersonaModal.vue`，顶部 Tab 切换两种创建方式：

**Tab 1：新建（蒸馏型）**
- 卡片网格展示所有可用配方（含名称、描述、`suitable_for` 标签、内置徽标）
- 单击卡片选中，双击或点底部"创建"确认
- 确认后调 `persona:create { recipe_name }` 创建占位 PersonaMeta：
  - `name`：默认 `未命名人格 HH:mm`（可在侧栏 inline 重命名）
  - `recipe_name`：用户选定；`source_type: 'distilled'`
  - `status: 'draft'`，无材料、无 SKILL.md
- 新条目立即出现在侧栏「草稿」组，自动选中并打开工作区

**Tab 2：导入（导入型）**
- 文件选择按钮（过滤 `.md` 文件）
- 选中后展示：文件名 + 完整路径 + 可滚动内容预览区
- 确认后顺序调用：
  1. `persona:create { source_type: 'imported', imported_from: filePath }`（`recipe_name` 存空字符串）
  2. `persona:save-skill-md { id, skillMd: content }` 写入文件内容
- 新条目出现在侧栏「草稿」组，工作区直接展示 SKILL.md 编辑器（内容已填充）

### 8.4 侧栏列表项

每个 PersonaItem 包含：
- 旋转图标（仅当该 Persona 处于活跃蒸馏中，导入型不会蒸馏故不出现）
- 名称（点击 ✎ 进入 inline 编辑模式，回车提交，Esc 取消）
- 蒸馏型：配方标签 + 材料数
- 导入型：`📥 已导入` 绿色徽标（取代配方标签和材料数）
- hover 显示重命名 / 删除按钮

### 8.5 工作区视图（PersonaDetail）

PersonaDetail 根据 `persona.source_type` 呈现两种模式：
...
| 区域 | 蒸馏型 | 导入型 |
|---|---|---|
| 头部右上角 | `🧪 配方名 ▾`（点击切换配方） | `📥 文件名`（只读，点击在资源管理器中定位文件目录） |
| 材料区 | 显示 | 隐藏 |
| SKILL.md 蒸馏控制 | 开始 / 重新蒸馏 / 中止 | 隐藏 |
| SKILL.md 编辑 / 发布 | 显示 | 显示 |
| 删除按钮 | 显示 | 显示 |

**头部右上角**：
- 蒸馏型：`🧪 配方名 ▾` 按钮，点击弹出 `RecipePickerModal` 卡片选择器
- 导入型：`📥 文件名` 只读徽标（绿色），鼠标悬停显示完整路径；点击调用 `openInExplorer` 打开文件所在目录（文件可能已移动，仅作参考）

**材料区添加输入**（仅蒸馏型）：用 Tab 切换三种来源（📝 文本 / 📂 文件 / 🌐 URL），默认选中「文本」。每个 Tab 内空间独立，文本框多行、URL 单行带错误提示、文件 Tab 居中放置大号选择按钮

**SKILL.md 卡片右上角**：保存修改、发布 / 撤销发布（两种模式均有）；蒸馏控制（开始 / 重新蒸馏 / 中止）仅蒸馏型显示
