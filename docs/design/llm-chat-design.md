...
| 工具调用 | 启用 Skill 后，LLM 可自动调用 SAFE / MODERATE 工具，MODERATE 工具需用户确认 |
...
```
sendMessage({ sessionId, userText, attachments, onEvent, enableTools })
  ├─ 抢占同 sessionId 的旧 requestId → abort()
  ├─ loadSession(sessionId)
  ├─ buildUserMessage() → appendMessages() 持久化
  └─ runStream() 异步执行：
       ├─ prepareLLMMessages()：image_ref 按 K=1 还原 / 历史淡出
       ├─ 构建 system blocks：session systemPrompt + Skill instructions
       ├─ while iter < 5:
...
| `chat:regenerate` | `{ sessionId, assistantMessageId }` | `{ requestId, discardedCount }` |
| `chat:edit-and-resend` | `ChatEditAndResendInput` | `{ requestId, userMessageId, discardedCount }` |
...
Chat Engine 默认通过 IPC 入口启用工具调用。每次请求开始时，从 SkillRegistry 动态获取当前启用工具列表，并把启用 Skill 的 instructions 拼接进 system prompt。
